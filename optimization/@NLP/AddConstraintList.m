...
function [obj] = AddConstraintList(obj, rbm, varargin)
...
if isfield(argIn, 'PolyType')
    ...
    error('''PolyType'' is a required argument for %s.', class(obj.Trajectory));
end
obj.Trajectory.PolyType = argIn.PolyType;

if isfield(argIn, 'PolyOrder')
    mustBeGreaterThanOrEqual(argIn.PolyOrder, 3)
else
    error('''PolyOrder'' is a required argument for %s.', class(obj.Trajectory));
end
obj.Trajectory.PolyOrder = argIn.PolyOrder;

if isfield(argIn, 'PolyPhase')
    mustBeMember(argIn.PolyPhase, {'time-based', 'state-based'})
else
    error('''PolyPhase'' is a required argument for %s.', class(obj.Trajectory));
end
obj.Trajectory.PolyPhase = argIn.PolyPhase;
...
switch obj.Trajectory.PolyType

    case 'Bezier'
        
        % define the matrix of Bezier coefficients
        alpha = AddBezierCoefficients(obj.Trajectory, rbm);
                
        % compute symbolic forms of desired trajectories and derivatives
        [phi, dphi, d2phi] = BezierTrajectory(obj.Trajectory, alpha, tau, t_minus, t_plus);
...
parameterization_type = obj.Trajectory.PolyPhase;


% controller outputs and time derivatives
[Y, DY, DDY] = ControllerOutput(obj.Trajectory,...
